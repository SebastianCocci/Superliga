...
/* ------------------- ICONOS (igual que tenías) ------------------- */
function MenuIcon({ isOpen }: { isOpen: boolean }) {
  return (
    <div className="w-6 h-6 flex flex-col justify-center gap-1.5">
      <div className={`w-full h-0.5 bg-white transition-transform ${isOpen ? "rotate-45 translate-y-2" : ""}`} />
      <div className={`w-full h-0.5 bg-white transition-opacity ${isOpen ? "opacity-0" : ""}`} />
      <div className={`w-full h-0.5 bg-white transition-transform ${isOpen ? "-rotate-45 -translate-y-2" : ""}`} />
    </div>
  )
}
...
// Icons: HomeIcon, TrophyIcon, etc. permanecen igual
